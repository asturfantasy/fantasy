...
async function loadLineup() {
  ...
  if (currentUser) {
    ...
    if (eg?.length) {
      ...
      if (!equipoGenerado) {
              if (window._generandoEquipoAuto) { return; }
              window._generandoEquipoAuto = true;
        ...
        window._generandoEquipoAuto = false;
      }
    }
  }
        ...
        const esCap = capitan !== null && String(capitan) === String(jugador.id);
                ...
                const overlay = sancionado
                          ? '<div style="position:absolute;inset:0;border-radius:50%;background:rgba(220,38,38,0.5);z-index:1;pointer-events:none;display:flex;align-items:center;justify-content:center;font-size:16px">🟥</div>'
                                          : duda
                                          ? '<div style="position:absolute;inset:0;border-radius:50%;background:rgba(255,140,0,0.45);z-index:1;pointer-events:none;display:flex;align-items:center;justify-content:center;font-size:16px">❓️</div>'
                                          : lesionado
                                          ? '<div style="position:absolute;inset:0;border-radius:50%;background:rgba(220,38,38,0.5);z-index:1;pointer-events:none;display:flex;align-items:center;justify-content:center;font-size:16px">🚑</div>'
                                          : '';
...
function openModal(slotId, posicion, cls) {
  ...
  list.innerHTML = '<div style="padding:12px 16px;border-bottom:1px solid var(--border);position:sticky;top:0;background:var(--bg2);z-index:1"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><span style="font-family:var(--font-mono);font-size:10px;color:var(--text-muted);letter-spacing:1px">PRESUPUESTO</span><span id="modal-presupuesto" style="font-family:var(--font-display);font-weight:700;font-size:16px;color:var(--neon)">' + getDisp() + 'M</span></div><div style="display:flex;gap:5px;margin-bottom:6px"><input id="modal-search" type="text" placeholder="Buscar..." style="flex:1;min-width:0;padding:7px 8px;font-family:var(--font-mono);font-size:16px;border:1px solid var(--border);border-radius:6px;background:var(--surface);color:var(--text)"><button id="btn-orden-puntos" style="flex-shrink:0;padding:7px 8px;background:var(--neon);color:#0d1117;border:none;border-radius:6px;font-family:var(--font-mono);font-weight:700;font-size:10px;cursor:pointer;white-space:nowrap;">Puntos</button><button id="btn-orden-valor" style="flex-shrink:0;padding:7px 8px;background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:6px;font-family:var(--font-mono);font-weight:700;font-size:10px;cursor:pointer;white-space:nowrap;">Valor</button><select id="modal-filtro-club" style="flex-shrink:0;width:60px;padding:7px 2px;font-family:var(--font-mono);font-size:10px;border:1px solid var(--border);border-radius:6px;background:var(--surface);color:var(--text)">' + opcionesClub + '</select></div><button id="btn-vaciar-posicion" style="width:100%;padding:7px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:6px;font-family:var(--font-display);font-weight:600;font-size:12px;cursor:pointer;margin-bottom:5px;">🗑 Vaciar posición</button><button id="btn-filtro-presupuesto" style="width:100%;padding:7px;background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:6px;font-family:var(--font-display);font-weight:600;font-size:12px;cursor:pointer;">Dentro del presupuesto</button></div><div id="modal-players"></div>';
      ...
      const clubLleno = !usado && (clubsCountSinActual[j.club] || 0) >= 2;
            ...
            const bR = sancionado ? '3px solid rgba(220,38,38,0.9)' : duda ? '3px solid rgba(255,140,0,0.9)' : lesionado ? '3px solid rgba(220,38,38,0.9)' : '1px solid var(--border)';
                  const badgeEstado = sancionado
                                      ...
                                      : duda
                                      ? '<span title="Duda" style="position:absolute;top:-3px;left:-3px;font-size:12px;background:var(--bg2);border-radius:50%;line-height:1">❓️</span>'
                                      : lesionado

...
function getCached(key, ttlMinutos = 5) {
  try {
    const item = localStorage.getItem('cache_' + key);
    if (!item) return null;
    const { data, timestamp } = JSON.parse(item);
    if (Date.now() - timestamp > ttlMinutos * 60 * 1000) { localStorage.removeItem('cache_' + key); return null; }
    return data;
  } catch(e) { return null; }
}

function setCache(key, data, ttlMinutos = 5) {
  try {
...
async function loadRanking() {
  ...
  // General — métricas rápidas (con caché)
  let general = getCached('clasificacion_general');
  if (!general) {
    const { data } = await db.from('clasificacion_general_auto').select('*');
    general = data;
    setCache('clasificacion_general', general);
  }
  if (general?.length) {
  ...
  if (equipoFav) {
    ...
    const penaFiltrada = (general || []).filter(r => userIdsFav.has(r.user_id));
...
async function loadRankingClasificacion() {
  ...
  if (equipoFav) {
    let penaAll = getCached('clasificacion_general');
    if (!penaAll) {
      const { data } = await db.from('clasificacion_general_auto').select('*');
      penaAll = data;
      setCache('clasificacion_general', penaAll);
    }
    const { data: equiposFav } = await db.from('equipos').select('user_id').eq('equipo_favorito', equipoFav);
    const userIdsFav = new Set((equiposFav || []).map(e => e.user_id));
...
async function verAlineacionUsuario(userId, nombreEquipo, jornada) {
  ...
  const { data, error } = await db.from('mi_equipo_detalle').select('*').eq('user_id', userId).eq('jornada', jornada).order('posicion');
  if (error || !data?.length) { content.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-muted)">Sin alineación para esta jornada</div>'; return; }
...
async function cargarMVPPreview(jornada) {
  ...
  if (!preview) return;
  const { data: partidosPublicados } = await db.from('partidos').select('jornada, local_abrev, visitante_abrev').eq('jornada', jornada).eq('publicado', true);
  if (!partidosPublicados?.length) { preview.innerHTML = '<div style="color:var(--text-muted);font-family:var(--font-mono);font-size:11px">Sin datos</div>'; return; }
  ...
  const { data: jugadores } = await db.from('jugadores').select('nombre, club, posicion, puntos, minutos, foto_url, escudo_url').eq('jornada', jornada).order('puntos', { ascending: false }).order('minutos', { ascending: true }).order('asistencia', { ascending: false });
...
async function cargarMVP(jornada) {
  ...
  const { data: partidosPublicados } = await db.from('partidos').select('jornada, local_abrev, visitante_abrev').eq('jornada', jornada).eq('publicado', true);
  if (!partidosPublicados?.length) { container.innerHTML = '<div style="text-align:center;padding:28px;color:var(--text-muted)">Sin datos para esta jornada</div>'; return; }
  const clubsPublicados = new Set();
  partidosPublicados.forEach(p => { clubsPublicados.add(p.local_abrev); clubsPublicados.add(p.visitante_abrev); });
  const { data: jugadores } = await db.from('jugadores').select('nombre, club, posicion, puntos, minutos, gol, penalti_marcado, asistencia, amarilla, roja, goles_encajados, foto_url, escudo_url, valor').eq('jornada', jornada).order('puntos', { ascending: false }).order('minutos', { ascending: true }).order('asistencia', { ascending: false });
  const mvp = (jugadores || []).find(j => clubsPublicados.has(j.club) && (j.minutos || 0) > 0);
  if (!mvp) { container.innerHTML = '<div style="text-align:center;padding:28px;color:var(--text-muted)">Sin datos para esta jornada</div>'; return; }
  window._mvpData = { mvp, jornada };
  const statsGrid = (bg) => `<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px"><div style="background:${bg};border-radius:8px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:white">${mvp.puntos}</div><div style="font-size:8px;color:rgba(255,255,255,0.5);letter-spacing:1px">PTS</div></div><div style="background:${bg};border-radius:8px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:white">${mvp.minutos||0}</div><div style="font-size:8px;color:rgba(255,255,255,0.5);letter-spacing:1px">MIN</div></div><div style="background:${bg};border-radius:8px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:white">${(mvp.gol||0)+(mvp.penalti_marcado||0)}</div><div style="font-size:8px;color:rgba(255,255,255,0.5);letter-spacing:1px">GOLES</div></div><div style="background:${bg};border-radius:8px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:white">${mvp.asistencia||0}</div><div style="font-size:8px;color:rgba(255,255,255,0.5);letter-spacing:1px">ASIST.</div></div></div>`;
  container.innerHTML =
    '<div style="background:var(--green-brand);border-radius:var(--radius);padding:20px;margin-bottom:16px;background-image:repeating-linear-gradient(90deg,transparent,transparent 40px,rgba(0,0,0,0.06) 40px,rgba(0,0,0,0.06) 80px),repeating-linear-gradient(0deg,transparent,transparent 40px,rgba(0,0,0,0.06) 40px,rgba(0,0,0,0.06) 80px)">' +
      ...
      '<div style="display:flex;align-items:center;gap:14px;margin-bottom:16px">' +
        ...
        '</div>' +
        '<div><div style="font-family:var(--font-display);font-size:22px;font-weight:800;color:white;line-height:1.1">' + mvp.nombre + '</div><div style="font-family:var(--font-mono);font-size:11px;color:rgba(255,255,255,0.6);margin-top:4px">' + mvp.posicion + ' · ' + mvp.club + '</div></div>' +
      '</div>' +
      statsGrid('rgba(0,0,0,0.2)') +
    '</div>' +
    '<button onclick="exportarMVP()" style="width:100%;padding:10px;background:var(--green-brand);color:white;border:none;border-radius:10px;font-family:var(--font-display);font-weight:700;font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px"><i class="ti ti-share"></i> Compartir MVP</button>';
}
...
async function exportarMVP() {
  ...
  tarjeta.innerHTML =
    '<div style="display:flex;align-items:center;gap:8px;margin-bottom:16px"><img src="https://rtmclmqzasktshlzwcyn.supabase.co/storage/v1/object/public/clubes/logo_asturfantasy_redondo.png" width="24" height="24" style="border-radius:6px"><span style="color:white;font-weight:700;font-size:13px">Astur<span style="color:rgba(255,255,255,0.7)">Fantasy</span></span><span style="margin-left:auto;font-size:11px;color:rgba(255,255,255,0.6)">J' + jornada + ' · MVP</span></div>' +
    '<div style="display:flex;align-items:center;gap:14px;margin-bottom:16px"><div style="width:64px;height:64px;border-radius:50%;background:rgba(255,255,255,0.2);border:2px solid rgba(255,255,255,0.4);display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;color:white;flex-shrink:0;overflow:hidden">' + (mvp.foto_url ? '<img src="' + mvp.foto_url + '" width="64" height="64" style="object-fit:cover;border-radius:50%">' : mvp.nombre.substring(0,2).toUpperCase()) + '</div><div><div style="font-size:9px;color:rgba(255,255,255,0.6);letter-spacing:2px;margin-bottom:2px">MVP JORNADA ' + jornada + '</div><div style="font-size:22px;font-weight:800;color:white;line-height:1.1">' + mvp.nombre + '</div><div style="font-size:11px;color:rgba(255,255,255,0.6);margin-top:2px">' + mvp.posicion + ' · ' + mvp.club + '</div></div></div>' +
    '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:14px"><div style="background:rgba(0,0,0,0.2);border-radius:8px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:white">' + mvp.puntos + '</div><div style="font-size:8px;color:rgba(255,255,255,0.5);letter-spacing:1px">PTS</div></div><div style="background:rgba(0,0,0,0.2);border-radius:8px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:white">' + (mvp.minutos||0) + '</div><div style="font-size:8px;color:rgba(255,255,255,0.5);letter-spacing:1px">MIN</div></div><div style="background:rgba(0,0,0,0.2);border-radius:8px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:white">' + ((mvp.gol||0)+(mvp.penalti_marcado||0)) + '</div><div style="font-size:8px;color:rgba(255,255,255,0.5);letter-spacing:1px">GOLES</div></div><div style="background:rgba(0,0,0,0.2);border-radius:8px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:white">' + (mvp.asistencia||0) + '</div><div style="font-size:8px;color:rgba(255,255,255,0.5);letter-spacing:1px">ASIST.</div></div></div>' +
  ...
  try {
    ...
    canvas.toBlob(async blob => {
      ...
      if (navigator.share && navigator.canShare({ files: [file] })) { await navigator.share({ files: [file], title: 'MVP J' + jornada + ' · AsturFantasy' }); }
      else { const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'MVP_J' + jornada + '.png'; a.click(); URL.revokeObjectURL(url); }
    });
  } catch(e) { if (document.body.contains(tarjeta)) document.body.removeChild(tarjeta); showToast('Error al compartir'); }
}

async function compartirClasificacion(nombreEquipo, posicion, puntos, tipo, club) {
  let tabla = [], titulo = '', subtitulo = '';
  if (tipo === 'jornada') {
    const jornadaRanking = jornadadCerrada() ? JORNADA_ACTIVA : JORNADA_VISIBLE;
    ...
    titulo = 'Jornada ' + jornadaRanking; subtitulo = 'Clasificación semanal';
  } else if (tipo === 'pena') {
    let penaAll = getCached('clasificacion_general');
    if (!penaAll) { const { data } = await db.from('clasificacion_general_auto').select('*'); penaAll = data; setCache('clasificacion_general', penaAll); }
    ...
    titulo = CLUBES_INFO[club]?.nombre || club || 'tu club'; subtitulo = 'Liga de peña';
  } else {
    let general = getCached('clasificacion_general');
    if (!general) { const { data } = await db.from('clasificacion_general_auto').select('*'); general = data; setCache('clasificacion_general', general); }
    tabla = (general || []).map((r, i) => ({ pos: i+1, nombre: r.nombre_equipo, pts: r.puntos_total, esYo: r.user_id === currentUser?.id }));
    titulo = 'Clasificación general'; subtitulo = 'AsturFantasy';
  }
  ...
  const filaHtml = (r, destacado = false) => '<div style="display:flex;align-items:center;gap:8px;padding:6px 12px;' + (destacado ? 'background:rgba(0,217,126,0.12);border-radius:6px;' : '') + '"><div style="width:20px;font-family:monospace;font-size:11px;font-weight:700;color:' + medalColor(r.pos) + ';flex-shrink:0;text-align:right">' + r.pos + '</div><div style="flex:1;font-size:11px;color:' + (destacado ? '#4cd97b' : 'white') + ';white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + r.nombre + '</div><div style="font-size:12px;font-weight:700;color:' + (destacado ? '#4cd97b' : 'white') + ';flex-shrink:0">' + r.pts + '</div></div>';
  const tarjeta = document.createElement('div');
  ...
  tarjeta.innerHTML =
    '<div style="display:flex;align-items:center;gap:8px;margin-bottom:14px"><img src="https://rtmclmqzasktshlzwcyn.supabase.co/storage/v1/object/public/clubes/logo_asturfantasy_redondo.png" width="22" height="22" style="border-radius:6px"><span style="color:white;font-weight:700;font-size:12px">Astur<span style="color:#4cd97b">Fantasy</span></span><span style="margin-left:auto;font-size:10px;color:rgba(255,255,255,0.4)">' + subtitulo + '</span></div>' +
    '<div style="font-size:15px;font-weight:800;color:white;margin-bottom:12px">' + titulo + '</div>' +
    '<div style="display:flex;flex-direction:column;gap:2px;margin-bottom:' + (!yoEnTop10 && yo ? '8px' : '14px') + '">' + top10.map(r => filaHtml(r, r.esYo)).join('') + '</div>' +
    (!yoEnTop10 && yo ? '<div style="border-top:1px dashed rgba(255,255,255,0.1);padding-top:8px;margin-bottom:14px"><div style="font-size:9px;color:rgba(255,255,255,0.3);letter-spacing:1px;margin-bottom:4px;padding-left:12px">TU POSICIÓN</div>' + filaHtml(yo, true) + '</div>' : '') +
  ...
  try {
    ...
    canvas.toBlob(async blob => {
      ...
      if (navigator.share && navigator.canShare({ files: [file] })) { await navigator.share({ files: [file], title: titulo + ' · AsturFantasy' }); }
      else { const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'clasificacion.png'; a.click(); URL.revokeObjectURL(url); }
    });
  } catch(e) { if (document.body.contains(tarjeta)) document.body.removeChild(tarjeta); showToast('Error al compartir'); }
}

async function loadRankingJugadores() {
  let jugadores = getCached('ranking_jugadores');
  if (!jugadores) {
    const { data } = await db.from('ranking_jugadores').select('*');
    jugadores = data;
    setCache('ranking_jugadores', jugadores);
  }
...
async function loadRankingDetalle() {
  rankingDetalleData = getCached('ranking_detalle');
  if (!rankingDetalleData) {
    ...
    setCache('ranking_detalle', rankingDetalleData);
  }
...
function cambiarSubtabDetalle(subtab) {
  ...
  const filtrados = (rankingDetalleData || []).filter(j => (j[campo] || 0) > 0).sort((a, b) => (b[campo] || 0) - (a[campo] || 0)).slice(0, 20);
  if (!filtrados.length) { container.innerHTML = '<div style="text-align:center;color:var(--text-muted);padding:28px;font-family:var(--font-mono);font-size:12px">Sin datos</div>'; return; }
  container.innerHTML = '<table class="ranking-table"><thead><tr><th>#</th><th>Jugador</th><th>Club</th><th style="text-align:right">' + icono + ' ' + label + '</th></tr></thead><tbody>' +
    filtrados.map((j, i) => '<tr class="' + medalClass(i+1) + '"><td><span class="rank-pos ' + medalClass(i+1) + '">' + (i+1) + '</span></td><td><div class="rank-name" style="cursor:pointer;text-decoration:underline" onclick="mostrarHistorial(\'' + j.nombre + '\',\'' + j.club + '\',\'' + j.posicion + '\')">' + j.nombre + '</div><div class="rank-team">' + j.posicion + '</div></td><td>' + (j.escudo_url ? '<img loading="lazy" src="' + j.escudo_url + '" width="22" height="22" style="object-fit:contain;vertical-align:middle;margin-right:4px">' : '') + '<span class="rank-team">' + j.club + '</span></td><td><div class="rank-pts">' + (j[campo] || 0) + '</div></td></tr>').join('') +
    '</tbody></table>';
}
...
async function loadPerfil() {
  ...
  const { data: clasificacion } = await db.from('clasificacion_automatica').select('jornada, puntos').eq('user_id', currentUser.id).order('jornada', { ascending: true });
  const { data: todasClasificaciones } = await db.from('clasificacion_automatica').select('user_id, jornada, puntos').order('jornada', { ascending: true });
  ...
  let generalAll = getCached('clasificacion_general');
  if (!generalAll) { const { data } = await db.from('clasificacion_general_auto').select('user_id, puntos_total').order('puntos_total', { ascending: false }); generalAll = data; setCache('clasificacion_general', generalAll); }
  ...
  const posSemanal = ultimaJornadaClasif ? todas.filter(j => j.jornada === ultimaJornadaClasif).sort((a,b) => b.puntos - a.puntos).findIndex(j => j.user_id === currentUser.id) + 1 : 0;
  ...
  jornadasUnicas.forEach(jornada => {
    ...
    if (pos > 0 && pos <= 10) { rachaTemp++; rachaMax = Math.max(rachaMax, rachaTemp); } else { rachaTemp = 0; }
  });
  for (let i = jornadasUnicas.length - 1; i >= 0; i--) {
    const jornada = jornadasUnicas[i];
    const ranking = todas.filter(j => j.jornada === jornada).sort((a,b) => b.puntos - a.puntos);
    const pos = ranking.findIndex(j => j.user_id === currentUser.id) + 1;
    if (pos > 0 && pos <= 10) rachaActual++; else break;
  }
  ...
  (misEquipos || []).forEach(e => { contadorJugadores[e.jugador_id] = (contadorJugadores[e.jugador_id] || 0) + 1; });
  const masUsadoId = Object.entries(contadorJugadores).sort((a,b) => b[1] - a[1])[0];
  let masUsadoNombre = '—', masUsadoVeces = 0;
  if (masUsadoId) { masUsadoVeces = masUsadoId[1]; const jugMasUsado = (jugadoresInfo || []).find(j => j.id === masUsadoId[0]); masUsadoNombre = jugMasUsado?.nombre || '—'; }
  ...
  if (masCapitanId) { masCapitanVeces = masCapitanId[1]; const jugCapitan = (jugadoresInfo || []).find(j => j.id === masCapitanId[0]); masCapitanNombre = jugCapitan?.nombre || '—'; }
  ...
  const agradecido = Object.values(jugadorPuntos).filter(j => j.veces >= 1).sort((a,b) => (b.pts/b.veces) - (a.pts/a.veces))[0];
  ...
  document.getElementById('perfil-stats').innerHTML = stats.map(s =>
    '<div style="background:var(--surface);border-radius:10px;padding:12px"><div style="font-family:var(--font-mono);font-size:9px;color:var(--text-muted);letter-spacing:1px;text-transform:uppercase;margin-bottom:6px">' + s.label + '</div><div style="font-family:var(--font-display);font-size:16px;font-weight:700;color:' + s.color + '">' + s.value + '</div><div style="font-family:var(--font-mono);font-size:10px;color:var(--text-muted);margin-top:2px">' + s.sub + '</div></div>'
  ).join('');

  const victoriaJornada = misJornadas.some(j => { const ranking = todas.filter(t => t.jornada === j.jornada).sort((a,b) => b.puntos - a.puntos); return ranking[0]?.user_id === currentUser.id; });
  const logros = [
  ...
  document.getElementById('perfil-logros').innerHTML = logros.map(l =>
    '<div style="display:flex;align-items:center;gap:12px;padding:12px;background:var(--surface);border-radius:10px;' + (!l.desbloqueado ? 'opacity:0.4;' : 'border:1px solid rgba(76,217,123,0.2);') + '"><div style="font-size:24px;flex-shrink:0">' + l.icono + '</div><div style="flex:1"><div style="font-family:var(--font-display);font-size:13px;font-weight:600;color:' + (l.desbloqueado ? 'var(--text)' : 'var(--text-muted)') + '">' + l.titulo + '</div><div style="font-family:var(--font-mono);font-size:10px;color:var(--text-muted);margin-top:2px">' + l.desc + '</div></div>' + (l.desbloqueado ? '<i class="ti ti-check" style="color:var(--neon);font-size:18px"></i>' : '<i class="ti ti-lock" style="color:var(--text-muted);font-size:16px"></i>') + '</div>'
  ).join('');
}
...
async function compartirPerfil() {
  ...
  tarjeta.innerHTML =
    '<div style="padding:16px;background:linear-gradient(135deg,#111816,#0d1f14);border-bottom:1px solid rgba(76,217,123,0.15)">' +
      '<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px"><img src="https://rtmclmqzasktshlzwcyn.supabase.co/storage/v1/object/public/clubes/logo_asturfantasy_redondo.png" width="22" height="22" style="border-radius:6px" onerror="this.style.display=\'none\'"><span style="color:white;font-weight:700;font-size:12px">Astur<span style="color:#4cd97b">Fantasy</span></span></div>' +
      '<div style="display:flex;align-items:center;gap:12px"><div style="width:48px;height:48px;border-radius:50%;background:rgba(76,217,123,0.15);border:2px solid rgba(76,217,123,0.4);display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:#4cd97b">' + nombreEquipo.substring(0,2).toUpperCase() + '</div><div><div style="font-size:16px;font-weight:700;color:white">' + nombreEquipo + '</div><div style="font-size:11px;color:rgba(255,255,255,0.5);margin-top:2px">' + clubFav + '</div></div></div>' +
    '</div>' +
    '<div style="padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.06)"><div style="font-size:9px;color:rgba(255,255,255,0.3);letter-spacing:1.5px;text-transform:uppercase;margin-bottom:10px">Estadísticas</div><div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px">' +
      stats.map(s => '<div style="background:rgba(255,255,255,0.05);border-radius:8px;padding:10px"><div style="font-size:8px;color:rgba(255,255,255,0.4);letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">' + s.label + '</div><div style="font-size:14px;font-weight:700;color:white">' + s.value + '</div><div style="font-size:9px;color:rgba(255,255,255,0.4);margin-top:2px">' + s.sub + '</div></div>').join('') +
    '</div></div>' +
    '<div style="padding:14px 16px"><div style="font-size:9px;color:rgba(255,255,255,0.3);letter-spacing:1.5px;text-transform:uppercase;margin-bottom:10px">Logros</div><div style="display:flex;flex-direction:column;gap:6px">' +
      logros.map(l => '<div style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:rgba(255,255,255,0.04);border-radius:8px;' + (l.desbloqueado ? 'border:1px solid rgba(76,217,123,0.2);' : 'opacity:0.4;') + '"><span style="font-size:18px">' + l.icono + '</span><span style="font-size:12px;font-weight:600;color:' + (l.desbloqueado ? 'white' : 'rgba(255,255,255,0.5)') + '">' + l.titulo + '</span>' + (l.desbloqueado ? '<span style="margin-left:auto;color:#4cd97b;font-size:14px">✓</span>' : '<span style="margin-left:auto;color:rgba(255,255,255,0.3);font-size:12px">🔒</span>') + '</div>').join('') +
    '</div></div>' +
    '<div style="padding:10px 16px;background:rgba(0,0,0,0.3);text-align:center"><span style="font-size:9px;color:rgba(255,255,255,0.3)">asturfantasy.com</span></div>';
  document.body.appendChild(tarjeta);
  try {
    ...
    canvas.toBlob(async blob => {
      ...
      if (navigator.share && navigator.canShare({ files: [file] })) { await navigator.share({ files: [file], title: nombreEquipo + ' · AsturFantasy' }); }
      else { const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = nombreEquipo + '_perfil.png'; a.click(); URL.revokeObjectURL(url); }
    });
  } catch(e) { if (document.body.contains(tarjeta)) document.body.removeChild(tarjeta); showToast('Error al compartir'); }
...
async function loadOnce(jornada) {
  ...
  let huecos = 3;
  for (const c of candidatos) { if (!huecos) break; if (c._pos==='DEF' && defOnce.length<5) { defOnce.push(c); huecos--; } else if (c._pos==='MED' && medOnce.length<4) { medOnce.push(c); huecos--; } else if (c._pos==='DEL' && delOnce.length<3) { delOnce.push(c); huecos--; } }
  ...
  container.innerHTML =
    '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><div style="font-family:var(--font-display);font-size:18px;font-weight:700;color:var(--neon);letter-spacing:2px">Formación: ' + defOnce.length + '-' + medOnce.length + '-' + delOnce.length + '</div><div style="font-family:var(--font-mono);font-size:11px;color:var(--text-muted)">TOTAL: <strong style="color:var(--neon)">' + totalPuntos + ' pts</strong></div></div>' +
    filas.map(fila => fila.jugadores.length === 0 ? '' :
      '<div style="margin-bottom:14px"><div style="font-family:var(--font-display);font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--text-muted);border-bottom:1px solid var(--border);padding-bottom:5px;margin-bottom:8px">' + fila.label + '</div><div style="display:flex;flex-direction:column;gap:4px">' +
      fila.jugadores.map(j => '<div style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:var(--surface);border-radius:8px"><div style="width:10px;height:10px;border-radius:50%;background:' + posColor[fila.pos] + ';flex-shrink:0"></div>' + (j.escudo_url ? '<img loading="lazy" src="' + j.escudo_url + '" width="20" height="20" style="object-fit:contain;flex-shrink:0">' : '') + '<div style="flex:1;font-family:var(--font-display);font-weight:600;font-size:13px;color:var(--text)">' + j.nombre + '</div><div style="font-family:var(--font-mono);font-size:10px;color:var(--text-muted)">' + j.club + '</div><div style="font-family:var(--font-display);font-weight:700;font-size:18px;color:var(--neon);min-width:28px;text-align:right">' + j.puntos + '</div></div>').join('') +
      '</div></div>'
    ).join('') +
...
async function compartirOnceIdeal() {
  ...
  tarjeta.style.cssText = 'position:fixed;top:-9999px;left:-9999px;width:360px;background:#111816;border-radius:16px;overflow:hidden;font-family:Space Grotesk,sans-serif;padding:20px;border:1px solid rgba(76,217,123,0.2)';
  tarjeta.innerHTML =
    '<div style="display:flex;align-items:center;gap:8px;margin-bottom:16px"><img src="https://rtmclmqzasktshlzwcyn.supabase.co/storage/v1/object/public/clubes/logo_asturfantasy_redondo.png" width="24" height="24" style="border-radius:6px"><span style="color:white;font-weight:700;font-size:13px">Astur<span style="color:#4cd97b">Fantasy</span></span><span style="margin-left:auto;font-size:11px;color:rgba(255,255,255,0.5)">J' + jornada + ' · Once ideal</span></div>' +
    ...
    filas.map(fila => fila.jugadores.length === 0 ? '' :
      '<div style="margin-bottom:10px"><div style="font-size:9px;color:#4a5e58;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px">' + fila.label + '</div><div style="display:flex;flex-direction:column;gap:3px">' +
      fila.jugadores.map(j => '<div style="background:#1a2420;border-radius:8px;padding:8px 12px;display:flex;align-items:center;gap:8px"><div style="width:10px;height:10px;border-radius:50%;background:' + posColor[fila.pos] + ';flex-shrink:0"></div>' + (j.escudo_url ? '<img src="' + j.escudo_url + '" width="16" height="16" style="object-fit:contain;flex-shrink:0">' : '') + '<span style="font-size:12px;font-weight:600;color:white;flex:1">' + j.nombre + '</span><span style="font-size:9px;color:#7a9088">' + j.club + '</span><span style="font-size:16px;font-weight:800;color:#4cd97b;min-width:24px;text-align:right">' + j.puntos + '</span></div>').join('') +
      '</div></div>'
    ).join('') +
  ...
  try {
    ...
    canvas.toBlob(async blob => {
      ...
      if (navigator.share && navigator.canShare({ files: [file] })) { await navigator.share({ files: [file], title: 'Once ideal J' + jornada + ' · AsturFantasy' }); }
      else { const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'once_ideal_J' + jornada + '.png'; a.click(); URL.revokeObjectURL(url); }
    });
  } catch(e) { if (document.body.contains(tarjeta)) document.body.removeChild(tarjeta); showToast('Error al compartir'); }
}
...
async function loadRankingRentable() {
  ...
  if (selectRentable && !selectRentable.options.length) {
    for (let i = JORNADA_ACTIVA; i >= 1; i--) { const opt = document.createElement('option'); opt.value = i; opt.textContent = 'Jornada ' + i; selectRentable.appendChild(opt); }
...
async function cargarRentable(jornada) {
  ...
  container.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
      ...
      <div style="font-family:var(--font-mono);font-size:11px;color:var(--text-muted)"><div>COSTE: <strong style="color:var(--amber)">${costeTotal.toFixed(1)}M</strong></div><div>PUNTOS: <strong style="color:var(--neon)">${puntosTotal} pts</strong></div></div>
    </div>
    ${filas.map(fila => fila.jugadores.length === 0 ? '' : `
      <div style="margin-bottom:14px">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${fila.jugadores.map(j => `<div style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:var(--surface);border-radius:8px"><div style="width:10px;height:10px;border-radius:50%;background:${posColor[fila.pos]};flex-shrink:0"></div>${j.escudo_url ? '<img loading="lazy" src="' + j.escudo_url + '" width="20" height="20" style="object-fit:contain;flex-shrink:0">' : ''}<div style="flex:1;font-family:var(--font-display);font-weight:600;font-size:13px;color:var(--text)">${j.nombre}</div><div style="font-family:var(--font-mono);font-size:10px;color:var(--text-muted)">${j.club} · ${j.valor}M</div><div style="text-align:right"><div style="font-family:var(--font-display);font-weight:700;font-size:16px;color:var(--neon)">${j.puntos} pts</div><div style="font-family:var(--font-mono);font-size:10px;color:var(--amber)">${j.rentabilidad.toFixed(2)} pts/M</div></div></div>`).join('')}
        </div>
      </div>`).join('')}
...
async function compartirOnceRentable() {
  ...
  tarjeta.innerHTML =
    '<div style="display:flex;align-items:center;gap:8px;margin-bottom:16px"><img src="https://rtmclmqzasktshlzwcyn.supabase.co/storage/v1/object/public/clubes/logo_asturfantasy_redondo.png" width="24" height="24" style="border-radius:6px"><span style="color:white;font-weight:700;font-size:13px">Astur<span style="color:#4cd97b">Fantasy</span></span><span style="margin-left:auto;font-size:11px;color:rgba(255,255,255,0.5)">J' + jornada + ' · Once rentable</span></div>' +
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px"><div style="background:#1a2420;border-radius:8px;padding:10px;text-align:center"><div style="font-size:9px;color:#7a9088;letter-spacing:1px;margin-bottom:2px">COSTE</div><div style="font-size:20px;font-weight:800;color:#e3b341">' + costeTotal.toFixed(1) + 'M</div></div><div style="background:#1a2420;border-radius:8px;padding:10px;text-align:center"><div style="font-size:9px;color:#7a9088;letter-spacing:1px;margin-bottom:2px">PUNTOS</div><div style="font-size:20px;font-weight:800;color:#4cd97b">' + puntosTotal + '</div></div></div>' +
    filas.map(fila => fila.jugadores.length === 0 ? '' :
      '<div style="margin-bottom:10px"><div style="font-size:9px;color:#4a5e58;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px">' + fila.label + '</div><div style="display:flex;flex-direction:column;gap:3px">' +
      fila.jugadores.map(j => '<div style="background:#1a2420;border-radius:8px;padding:8px 12px;display:flex;align-items:center;gap:8px"><div style="width:10px;height:10px;border-radius:50%;background:' + posColor[fila.pos] + ';flex-shrink:0"></div>' + (j.escudo_url ? '<img src="' + j.escudo_url + '" width="16" height="16" style="object-fit:contain;flex-shrink:0">' : '') + '<span style="font-size:12px;font-weight:600;color:white;flex:1">' + j.nombre + '</span><span style="font-size:9px;color:#7a9088">' + j.valor + 'M</span><span style="font-size:12px;font-weight:700;color:#e3b341;min-width:40px;text-align:right">' + j.rentabilidad.toFixed(2) + '</span></div>').join('') +
      '</div></div>'
    ).join('') +
    '<div style="margin-top:12px;text-align:center;font-size:10px;color:#4a5e58">asturfantasy.com</div>';
  document.body.appendChild(tarjeta);
  try {
    ...
    canvas.toBlob(async blob => {
      ...
      if (navigator.share && navigator.canShare({ files: [file] })) { await navigator.share({ files: [file], title: 'Once rentable J' + jornada + ' · AsturFantasy' }); }
      else { const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'once_rentable_J' + jornada + '.png'; a.click(); URL.revokeObjectURL(url); }
    });
  } catch(e) { if (document.body.contains(tarjeta)) document.body.removeChild(tarjeta); showToast('Error al compartir'); }
}
...
async function loadLigas() {
  ...
  container.innerHTML =
    '<div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap">' +
      ...
      '<button onclick="mostrarUnirLiga()" style="flex:1;padding:10px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:8px;font-family:var(--font-display);font-weight:700;font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px"><i class="ti ti-door-enter"></i> Unirse</button>' +
    '</div>' +
    ...
    (misLigas.length === 0
      ...
      : '<div id="ligas-cards">' + misLigas.map(l => ligaCardHtml(l)).join('') + '</div>'
    ) +
  ...
  return '<div class="liga-card" id="liga-card-' + l.id + '" style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;margin-bottom:10px;overflow:hidden">' +
    ...
    '<div onclick="toggleLigaCard(\'' + l.id + '\')" style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;cursor:pointer">' +
      ...
      '<div style="display:flex;align-items:center;gap:6px">' +
        '<button onclick="event.stopPropagation();copiarCodigo(\'' + l.codigo + '\')" style="padding:5px 8px;background:var(--surface);border:1px solid var(--border);border-radius:6px;color:var(--text-muted);cursor:pointer;font-size:12px"><i class="ti ti-copy"></i></button>' +
...
function toggleLigaCard(ligaId) {
  const detail = document.getElementById('liga-detail-' + ligaId);
  const chevron = document.getElementById('chevron-liga-' + ligaId);
  if (!detail) return;
...
function mostrarUnirLiga() {
  ...
  form.innerHTML =
    '<div style="background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:16px">' +
      ...
      '<div style="margin-bottom:10px">' +
        '<div style="font-family:var(--font-mono);font-size:10px;color:var(--text-muted);letter-spacing:1px;margin-bottom:6px">CÓDIGO</div>' +
        '<input id="codigo-liga" type="text" placeholder="AB3X7Z" maxlength="6" style="width:100%;padding:8px 12px;background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:8px;font-family:var(--font-mono);font-size:16px;letter-spacing:4px;text-transform:uppercase">' +
      '</div>' +
      '<div style="display:flex;gap:8px">' +
...
async function crearLiga() {
  ...
  const { data: liga, error } = await db.from('ligas').insert({ nombre, codigo, creador_id: currentUser.id }).select().single();
  if (error) {
    if (error.message.includes('ligas_nombre_unique')) {
      showToast('Ya existe una liga con ese nombre', true);
    } else {
      showToast('Error al crear la liga', true);
    }
    return;
  }

...
function updateBottomNav(screenId) {
  ...
  if (nav) nav.style.display = screenId === 'criterios' ? 'none' : 'flex';
}
...
async function abrirConsultaPuntos() {
  ...
  select.onchange = async () => {
    ...
    const { data, error } = await db.from('jugadores')
      ...
      .eq('club', club).eq('jornada', JORNADA_VISIBLE)
    ...
    const renderFila = (j) => `
      <div class="fila-jugador-puntos" data-nombre="${j.nombre.toLowerCase()}"
           style="display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:1px solid var(--border);cursor:pointer;width:100%"
           onclick="mostrarDesglose(${JSON.stringify(j).replace(/"/g, '&quot;')})">
        <div style="position:relative;width:36px;height:36px;flex-shrink:0">
          ${j.foto_url
            ? `<img loading="lazy" src="${j.foto_url}" width="36" height="36" style="object-fit:cover;border-radius:50%;border:1px solid var(--border)" onerror="this.style.display='none'">`
            : `<div style="width:36px;height:36px;border-radius:50%;background:var(--surface);display:flex;align-items:center;justify-content:center;font-family:var(--font-display);font-size:13px;color:var(--text-muted)">${j.nombre.substring(0,2).toUpperCase()}</div>`}
          ${j.escudo_url ? `<img loading="lazy" src="${j.escudo_url}" width="14" height="14" style="position:absolute;bottom:-2px;right:-2px;object-fit:contain;border-radius:50%;background:white;border:1px solid rgba(0,0,0,0.2)">` : ''}
        </div>
    ...
    content.innerHTML = `
      <input id="buscar-jugador-puntos" type="text" placeholder="Buscar jugador..."
        style="width:100%;padding:8px 12px;font-family:var(--font-mono);font-size:13px;background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:8px;margin-bottom:12px;box-sizing:border-box;position:sticky;top:0;z-index:1;">
      <div id="lista-jugadores-puntos" style="width:100%">${sorted.map(renderFila).join('')}</div>`;
...
document.getElementById('puntos-jornada-close')?.addEventListener('click', () => document.getElementById('modal-puntos-jornada').classList.remove('open'));
...
async function mostrarHistorial(nombre, club, posicion) {
  ...
  content.innerHTML = `
    ...
    <div style="font-family:var(--font-mono);font-size:11px;color:var(--text-muted);text-align:right;margin-bottom:14px;letter-spacing:1px">
      TOTAL: <strong style="color:var(--neon)">${total} pts</strong>
    </div>
    ${data.map(d => `
...
async function mostrarPartido(localAbrev, visitanteAbrev, localNombre, visitanteNombre) {
  ...
  const marcador = resultadoPartido?.resultado?.finalizado ? ` ${resultadoPartido.resultado.local} - ${resultadoPartido.resultado.visitante}` : '';
  ...
  const { data, error } = await db.from('jugadores')
    ...
    .in('club', [localAbrev, visitanteAbrev]).eq('jornada', JORNADA_ACTIVA)
    .order('total_jornada', { ascending: false });
  if (error || !data?.length) {
    content.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-muted)">Sin datos</div>';
    return;
  ...
  const renderJugador = (j, alineacion = 'left') => `
    <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border);cursor:pointer;flex-direction:${alineacion === 'right' ? 'row-reverse' : 'row'}"
         onclick="mostrarDesglose(${JSON.stringify(j).replace(/"/g, '&quot;')})">
      <div style="position:relative;width:30px;height:30px;flex-shrink:0">
        ${j.foto_url ? `<img loading="lazy" src="${j.foto_url}" width="30" height="30" style="object-fit:cover;border-radius:50%;border:1px solid var(--border)" onerror="this.style.display='none'">` : `<div style="width:30px;height:30px;border-radius:50%;background:var(--surface);display:flex;align-items:center;justify-content:center;font-family:var(--font-display);font-size:10px;color:var(--text-muted)">${j.nombre.substring(0,2).toUpperCase()}</div>`}
        ${j.escudo_url ? `<img loading="lazy" src="${j.escudo_url}" width="11" height="11" style="position:absolute;bottom:-1px;right:-1px;object-fit:contain;border-radius:50%;background:white;border:1px solid rgba(0,0,0,0.2)">` : ''}
      </div>
      <div style="flex:1;min-width:0;text-align:${alineacion === 'right' ? 'right' : 'left'}">
        <div style="font-family:var(--font-display);font-weight:600;font-size:12px;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${j.nombre}</div>
        <div style="font-family:var(--font-mono);font-size:9px;color:var(--text-muted)">${j.posicion}</div>
      </div>
      <div style="font-family:var(--font-display);font-weight:700;font-size:15px;color:var(--neon);flex-shrink:0">${j.total_jornada}</div>
    </div>`;
  ...
  content.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;max-height:60vh;overflow-y:auto;padding-right:4px">
      <div>
        <div style="font-family:var(--font-display);font-weight:700;font-size:13px;color:var(--text);text-align:center;margin-bottom:8px;padding-bottom:6px;border-bottom:2px solid var(--neon);position:sticky;top:0;background:var(--bg2);z-index:1">
  ...
  content.innerHTML = items.length ? `
    ${items.map(item => `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--border)">
        ...
        <span style="font-family:var(--font-display);font-weight:700;font-size:15px;color:${item.pts >= 0 ? 'var(--neon)' : 'var(--red)'}">${item.pts > 0 ? '+' : ''}${item.pts}</span>
      </div>`).join('')}
    <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;margin-top:4px">
      <span style="font-family:var(--font-display);font-weight:700;font-size:13px;text-transform:uppercase;letter-spacing:1px;color:var(--text)">Total</span>
      <span style="font-family:var(--font-display);font-weight:700;font-size:24px;color:var(--neon)">${j.total_jornada}</span>
    </div>` : `<div style="text-align:center;padding:20px;color:var(--text-muted)">Sin puntuación esta jornada</div>`;
  modal.classList.add('open');
}
...
async function loadHome() {
  ...
  const { data: sub } = await db.from('push_subscriptions').select('id')
  ...
  if (bannerAviso) {
    if (window.MENSAJE_AVISO) { bannerAviso.textContent = window.MENSAJE_AVISO; bannerAviso.style.display = 'block'; }
    else bannerAviso.style.display = 'none';
  }
  ...
  if (currentUser) {
    ...
    if (clSemanal?.puntos !== undefined) {
      ...
      if (deltaEl) {
        deltaEl.textContent = diff >= 0 ? `+${diff} vs media` : `${diff} vs media`;
        deltaEl.style.color = diff >= 0 ? 'var(--neon)' : 'var(--red)';
      }
  ...
  if (!PARTIDOS.length) {
    container.innerHTML = '<div style="text-align:center;padding:32px 20px;font-family:var(--font-display);font-size:15px;color:var(--text-muted);letter-spacing:1px">Próxima jornada por confirmar</div>';
    return;
  }
    ...
    <div class="match-card">
      <div class="match-team">
        <div class="crest" style="color:white;display:flex;align-items:center;justify-content:center">
          ${p.local.escudo_url ? `<img loading="lazy" src="${p.local.escudo_url}" alt="${p.local.abrev}" width="44" height="44" style="object-fit:contain" onerror="this.outerHTML='${p.local.abrev}'">` : p.local.abrev}
        </div>
        <div><div class="team-name">${p.local.nombre}</div></div>
      </div>
      <div style="display:flex;flex-direction:column;align-items:center;text-align:center;gap:2px;padding:0 8px;min-width:90px;max-width:110px">
        ${p.resultado?.finalizado ? `
          <div style="font-family:var(--font-display);font-size:18px;font-weight:700;color:var(--text);letter-spacing:2px">${p.resultado.local} - ${p.resultado.visitante}</div>
          <div style="font-family:var(--font-mono);font-size:9px;letter-spacing:2px;color:var(--neon);text-transform:uppercase;margin-bottom:2px">Final</div>` : `
        ...
        ${p.resultado?.finalizado ? `
          <button onclick="mostrarPartido('${p.local.abrev}', '${p.visitante.abrev}', '${p.local.nombre}', '${p.visitante.nombre}')"
            style="background:var(--neon);color:#0d1117;border:none;border-radius:20px;padding:5px 12px;cursor:pointer;font-family:var(--font-display);font-weight:700;font-size:9px;letter-spacing:1px;text-transform:uppercase;margin-top:4px;width:100%">
            Ver puntos
          </button>` : ''}
      </div>
      <div class="match-team right">
        <div class="crest" style="color:white;display:flex;align-items:center;justify-content:center">
          ${p.visitante.escudo_url ? `<img loading="lazy" src="${p.visitante.escudo_url}" alt="${p.visitante.abrev}" width="44" height="44" style="object-fit:contain" onerror="this.outerHTML='${p.visitante.abrev}'">` : p.visitante.abrev}
        </div>
        <div style="text-align:right"><div class="team-name">${p.visitante.nombre}</div></div>
      </div>
    </div>`).join('');
}
...
async function loadLineup() {
  ...
  if (deadlineEl) {
    ...
    if (jornadadCerrada()) {
      ...
      deadlineEl.innerHTML = `
        ...
        <span class="deadline-abierto-card" style="margin-top:6px;margin-bottom:8px" id="countdown-box">
          <span id="countdown-timer">Calculando...</span>
        </span>`;
      const actualizarCuenta = () => {
        const diff = new Date(DEADLINE_JORNADA) - new Date();
        if (diff <= 0) { document.getElementById('countdown-timer').textContent = '¡Plazo cerrado!'; clearInterval(intervalo); return; }
  ...
  if (jornadadCerrada()) {
    ...
    pitch.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:300px;gap:16px;position:relative;z-index:1">
        <div style="font-size:48px">🔒</div>
        <div style="font-family:var(--font-display);font-size:26px;font-weight:700;color:white;letter-spacing:2px;text-align:center">JORNADA CERRADA</div>
        <div style="font-family:var(--font-mono);font-size:11px;color:rgba(255,255,255,0.5);text-align:center;letter-spacing:1px">La J${JORNADA_ACTIVA} ya ha empezado<br>Es tarde para modificar tu alineación</div>
      </div>`;
  ...
  if (currentUser) {
    const { data: equipoGuardado } = await db.from('mi_equipo').select('jugador_id, formacion, capitan').eq('user_id', currentUser.id).eq('jornada', JORNADA_ACTIVA);
    if (equipoGuardado?.length) {
      ...
      const ordenPos = ['POR','DEF','MED','DEL','ENT'];
      jugadoresGuardados.sort((a,b) => ordenPos.indexOf(a.posicion) - ordenPos.indexOf(b.posicion))
      ...
      if (capitanIdGuardado) document.getElementById('capitan-select').value = capitanIdGuardado;
...
async function exportarAlineacion() {
  ...
  const { data: equipoData } = await db.from('equipos').select('nombre_equipo').eq('user_id', currentUser.id).single();
...
function renderPitch() {
  ...
  const filas = [
    { pos:'DEL', count:fwd, cls:'fwd' }, { pos:'MED', count:mid, cls:'mid' },
    { pos:'DEF', count:def, cls:'def' }, { pos:'POR', count:1, cls:'gk' }, { pos:'ENT', count:1, cls:'ent' },
  ];
  filas.forEach(fila => {
    ...
    for (let i = 0; i < fila.count; i++) {
      ...
      if (jugador) {
        ...
        const overlayRojo = noDisponible ? `<div style="position:absolute;inset:0;border-radius:50%;background:rgba(220,38,38,0.5);z-index:1;pointer-events:none"></div>` : '';
        const circuloContenido = jugador.foto_url ? `<img loading="lazy" src="${jugador.foto_url}" alt="${jugador.nombre}" width="46" height="46" style="object-fit:cover;border-radius:50%" onerror="this.style.display='none'">` : jugador.nombre.substring(0,3).toUpperCase();
        const escudoCirculo = jugador.escudo_url ? `<img loading="lazy" src="${jugador.escudo_url}" alt="${jugador.club}" width="16" height="16" style="position:absolute;bottom:-2px;right:-2px;object-fit:contain;border-radius:50%;background:white;border:1px solid rgba(0,0,0,0.2);z-index:2">` : '';
        slot.innerHTML = `
          <div class="player-circle ${fila.cls} ${esCap ? 'es-capitan' : ''}" style="overflow:visible;position:relative;${noDisponible ? 'border:3px solid rgba(220,38,38,0.9);' : ''}">
            ${circuloContenido}${overlayRojo}${escudoCirculo}
...
function openModal(slotId, posicion, cls) {
  ...
  const getPresupuestoDisponible = () => (PRESUPUESTO - Object.values(seleccionados).reduce((acc, j) => acc + (j.valor || 0), 0)).toFixed(1);
  let soloDisponibles = false;
  list.innerHTML = `
    <div style="padding:12px 16px;border-bottom:1px solid var(--border);position:sticky;top:0;background:var(--bg2);z-index:1">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        <span style="font-family:var(--font-mono);font-size:10px;color:var(--text-muted);letter-spacing:1px">PRESUPUESTO</span>
        <span id="modal-presupuesto" style="font-family:var(--font-display);font-weight:700;font-size:16px;color:var(--neon)">${getPresupuestoDisponible()}M</span>
      </div>
      <input id="modal-search" type="text" placeholder="Buscar jugador..."
        style="width:100%;padding:7px 10px;font-family:var(--font-mono);font-size:12px;border:1px solid var(--border);border-radius:6px;background:var(--surface);color:var(--text);margin-bottom:6px">
      <button id="btn-vaciar-posicion" style="width:100%;padding:7px;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:6px;font-family:var(--font-display);font-weight:600;font-size:12px;cursor:pointer;">🗑 Vaciar posición</button>
      <button id="btn-filtro-presupuesto" style="width:100%;padding:7px;background:var(--neon);color:#0d1117;border:none;border-radius:6px;font-family:var(--font-display);font-weight:700;font-size:12px;cursor:pointer;margin-top:5px;">Dentro del presupuesto</button>
    </div>
  ...
  document.getElementById('btn-vaciar-posicion').addEventListener('click', () => {
    ...
    closeModal(); renderPitch(); actualizarSelectCapitan(); actualizarPresupuesto();
  });
  ...
  const renderLista = (filtro = '') => {
    ...
    const filtrados = jugadoresPorPos[posicion].filter(j =>
      (j.nombre.toLowerCase().includes(filtro.toLowerCase()) || j.club.toLowerCase().includes(filtro.toLowerCase())) &&
    ...
    document.getElementById('modal-players').innerHTML = filtrados.map(j => {
      ...
      const bordeRojo = noDisponible ? '3px solid rgba(220,38,38,0.9)' : '1px solid var(--border)';
      const escudo = `
        <div style="position:relative;width:36px;height:36px;flex-shrink:0">
          ${j.foto_url ? `<img loading="lazy" src="${j.foto_url}" width="36" height="36" style="object-fit:cover;border-radius:50%;border:${bordeRojo}" onerror="this.style.display='none'">` : `<div style="width:36px;height:36px;border-radius:50%;background:${colores[cls]};color:${textoCols[cls]};display:flex;align-items:center;justify-content:center;font-family:var(--font-display);font-size:12px;border:${bordeRojo}">${j.nombre.substring(0,2).toUpperCase()}</div>`}
          ${j.escudo_url ? `<img loading="lazy" src="${j.escudo_url}" width="13" height="13" style="position:absolute;bottom:-2px;right:-2px;object-fit:contain;border-radius:50%;background:white;border:1px solid rgba(0,0,0,0.2)">` : ''}
        </div>`;
      return `<div class="modal-player" data-id="${j.id}" data-slot="${slotId}" style="opacity:${usado ? 0.3 : 1};pointer-events:${usado ? 'none' : 'auto'}">
        ${escudo}
        <div><div class="modal-player-name">${j.nombre}</div><div class="modal-player-meta">${j.club} · ${j.posicion}${j.rival ? ` · vs ${j.rival} (${j.es_local ? '🏠' : '✈️'})` : ''}</div></div>
        <div style="text-align:right"><div class="modal-player-pts">${posicion === 'ENT' ? (j.puntos_entrenador || 0) : j.puntos_total}</div><div style="font-family:var(--font-mono);font-size:10px;color:var(--amber)">${j.valor || 0}M</div></div>
      </div>`;
    }).join('');
...
document.getElementById('formation-select').addEventListener('change', () => {
  ...
  Object.keys(seleccionados).forEach(slotId => {
    ...
    if (parseInt(idx) >= maxPorPos[pos]) {
      if (capitan === seleccionados[slotId]?.id) { capitan = null; const sel = document.getElementById('capitan-select'); if (sel) sel.value = ''; }
      delete seleccionados[slotId];
    }
  });
...
document.getElementById('capitan-select')?.addEventListener('change', e => {
  if (e.isTrusted) { capitan = e.target.value || null; cambiosSinGuardar = true; renderPitch(); }
});
...
document.getElementById('btn-save-lineup').addEventListener('click', async () => {
  ...
  const formacion = document.getElementById('formation-select').value;
  const { def, mid, fwd } = FORMACIONES[formacion];
  const totalSlots = 1 + def + mid + fwd + 1;
  if (Object.keys(seleccionados).length < totalSlots) { showToast('¡Faltan jugadores por seleccionar!', true); return; }
  const gastado = Object.values(seleccionados).reduce((acc, j) => acc + (j.valor || 0), 0);
  if (gastado > PRESUPUESTO) { showToast('Has superado el presupuesto', true); return; }
  ...
  const filas = Object.values(seleccionados).map(jugador => ({ user_id: currentUser.id, jugador_id: jugador.id, jornada: JORNADA_ACTIVA, formacion, capitan: capitan === jugador.id }));
...
async function mostrarDesgloseMyTeam(jugadorId, nombre, posicion, jornada = JORNADA_VISIBLE) {
  ...
  const { data, error } = await db.from('jugadores')
    ...
    .eq('id', jugadorId).eq('jornada', jornada).single();
  ...
  content.innerHTML = `
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;margin-top:4px">
      <span style="font-family:var(--font-display);font-weight:700;font-size:13px;text-transform:uppercase;letter-spacing:1px;color:var(--text)">Total</span>
      <span style="font-family:var(--font-display);font-weight:700;font-size:24px;color:var(--neon)">${data.total_jornada}</span>
    </div>`;
}
...
async function cargarMyTeam(jornada) {
  ...
  const { data: capData } = await db.from('mi_equipo').select('jugador_id, capitan').eq('user_id', currentUser.id).eq('jornada', jornada).eq('capitan', true).single();
  ...
  grid.innerHTML = sorted.map(j => {
    ...
    const avatarContenido = foto ? `<img loading="lazy" src="${foto}" width="40" height="40" style="object-fit:cover;border-radius:50%" onerror="this.style.display='none'">` : j.nombre.substring(0,2).toUpperCase();
    const esCapitan = j.jugador_id === capitanId;
    const puntosFinales = esCapitan ? j.puntos * 2 : j.puntos;
    return `<div class="player-card ${esCapitan ? 'card-capitan' : ''}" style="cursor:pointer" onclick="mostrarDesgloseMyTeam('${j.jugador_id}', '${j.nombre}', '${j.posicion}', ${jornada})">
      ...
      <div class="pc-info">
        ...
        <div class="pc-meta">${j.posicion} · ${j.club} · ${valorMap[j.jugador_id] || 0}M${esCapitan ? ' · Cap.' : ''}</div>
      </div>
      <div class="pc-pts">${puntosFinales}</div>
    </div>`;
  }).join('');
  const { data: equipoData } = await db.from('equipos').select('nombre_equipo').eq('user_id', currentUser.id).single();
  const inputNombre = document.getElementById('input-nombre-equipo');
  if (inputNombre && equipoData?.nombre_equipo) inputNombre.value = equipoData.nombre_equipo;
}
    ...
    tbodyGeneral.innerHTML = general.map((r, i) => {
      ...
      return `
        <tr class="${medalClass(i+1)}" style="${esYo ? 'outline: 2px solid var(--neon);outline-offset:-2px;' : ''}">
          <td><span class="rank-pos ${medalClass(i+1)}">${i+1}</span></td>
          <td><div class="rank-team">${esYo ? '⭐ ' : ''}${r.nombre_equipo}</div></td>
          <td>
            <div style="display:flex;align-items:center;gap:8px;justify-content:flex-end">
              <div class="rank-pts">${r.puntos_total}</div>
              ${esYo ? `<button onclick="compartirClasificacion('${r.nombre_equipo}', ${i+1}, ${r.puntos_total})" style="background:var(--neon);color:#0d1117;border:none;border-radius:20px;padding:4px 10px;cursor:pointer;font-family:var(--font-display);font-weight:700;font-size:10px;white-space:nowrap">COMPARTIR</button>` : ''}
            </div>
          </td>
        </tr>`;
    }).join('');
  ...
  renderJugadoresFn = () => {
    ...
    tbody.innerHTML = paginados.map((j, i) => `
      <tr class="${medalClass(inicio + i + 1)}">
        ...
        <td>
          ${j.escudo_url ? `<img loading="lazy" src="${j.escudo_url}" width="22" height="22" style="object-fit:contain;vertical-align:middle;margin-right:4px" onerror="this.style.display='none'">` : ''}
    ...
    if (paginador) {
      paginador.innerHTML = totalPaginas > 1 ? `
        <div style="display:flex;justify-content:center;align-items:center;gap:12px;padding:14px 0">
          <button onclick="cambiarPagina(-1)" style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:6px 14px;cursor:pointer;font-family:var(--font-display);color:var(--text)" ${paginaActual === 1 ? 'disabled' : ''}>← Anterior</button>
          <span style="font-family:var(--font-mono);font-size:12px;color:var(--text-muted)">${paginaActual} / ${totalPaginas}</span>
          <button onclick="cambiarPagina(1)" style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:6px 14px;cursor:pointer;font-family:var(--font-display);color:var(--text)" ${paginaActual === totalPaginas ? 'disabled' : ''}>Siguiente →</button>
        </div>` : '';
    }
  };
  renderJugadoresFn();
...
async function guardarNombreEquipo() {
  ...
  if (error) {
    if (error.message.includes('equipos_nombre_equipo_unique')) showToast('Ese nombre ya está en uso', true);
    else showToast('Error al guardar: ' + error.message, true);
...
async function loadOnce(jornada) {
  ...
  const { data, error } = await db.from('jugadores')
    ...
    .order('puntos', { ascending: false }).order('valor', { ascending: true });
  ...
  const candidatos = [...defs.slice(3, 5).map(j => ({ ...j, _pos: 'DEF' })), ...meds.slice(3, 4).map(j => ({ ...j, _pos: 'MED' })), ...dels.slice(1, 3).map(j => ({ ...j, _pos: 'DEL' }))].sort((a, b) => b.puntos - a.puntos || a.valor - b.valor);
  let huecos = 3;
  ...
  if (totalPuntos === 0) { container.innerHTML = '<div style="text-align:center;padding:40px 20px;color:var(--text-muted);font-family:var(--font-display);font-size:16px">Aún no tenemos el once de la jornada</div>'; return; }
  const formacionOnce = `${defOnce.length}-${medOnce.length}-${delOnce.length}`;
  container.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
      <div style="font-family:var(--font-display);font-size:18px;font-weight:700;color:var(--neon);letter-spacing:2px">Formación: ${formacionOnce}</div>
      ...
      <div style="margin-bottom:18px">
        ...
        ${fila.jugadores.map(j => `
          <div style="display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:1px solid var(--border)">
            ${j.escudo_url ? `<img loading="lazy" src="${j.escudo_url}" width="26" height="26" style="object-fit:contain" onerror="this.style.display='none'">` : `<div style="width:26px;height:26px;background:var(--surface);border-radius:50%;display:flex;align-items:center;justify-content:center;font-family:var(--font-display);font-size:9px">${j.club}</div>`}
            <div style="flex:1">
              <div style="font-family:var(--font-display);font-weight:600;font-size:14px;color:var(--text)">${j.nombre}</div>
              <div style="font-family:var(--font-mono);font-size:10px;color:var(--text-muted)">${j.club} · ${j.posicion}</div>
            </div>
            <div style="font-family:var(--font-display);font-weight:700;font-size:20px;color:var(--neon)">${j.puntos}</div>
          </div>`).join('')}
      </div>`).join('')}`;
}
...
(async function init() {
  ...
  if (session?.user) {
    ...
    const { data: equipoData } = await db.from('equipos').select('nombre_equipo').eq('user_id', currentUser.id).single();
    if (!equipoData?.nombre_equipo) document.getElementById('modal-nombre-equipo').classList.add('open');
    else goTo('home');
  }
})();